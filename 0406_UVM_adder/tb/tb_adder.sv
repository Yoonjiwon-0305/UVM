...
class adder_sequence extends uvm_sequence;
    `uvm_object_utils(adder_sequence)  //factory 에 adder_sequence 등록 macro
...
class adder_monitor extends uvm_monitor;
    ...
    virtual function void build_phase(uvm_phase phase);
        ...
        if (!uvm_config_db#(virtual adder_if)::get(
            ...
            `uvm_fatal(get_name(), "Unalbe to access adder interface");
        end
    endfunction
...
endclass  //adder_agent extends superClass

class adder_environment extends uvm_env;
    `uvm_component_utils(
        adder_environment)  // factory에 adder_environment 등록 macro
...
class adder_test extends uvm_test;
    `uvm_component_utils(adder_test)  // factory에 adder_test를 등록 

    adder_sequence a_seq;
    adder_environment a_env;  // 멤버 변수

    function new(string name = "ADDER_TEST", uvm_component c);
        super.new(name, c);  // 부모에게 이름을 알려주는 용도
    endfunction
