...
class ram_seq_item extends uvm_sequence_item;
    ...
    function string conver2string();
        return $sformatf(
            "reset_n=%0b write=%0b addr=%0h wdata=%0h rdata=%0h",
            reset_n,
            write,
            addr,
            wdata,
...
class ram_monitor extends uvm_monitor;
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            if (!item.write && item.reset_n) begin
                @(posedge r_if.clk);
                #2;
                item.rdata = r_if.rdata;
            end
...
class ram_coverage extends uvm_subscriber #(ram_seq_item);
    ...
    covergroup ram_cg;
        ...
        cx_rst_write   : cross cp_reset_n, cp_write;
    endgroup
    ...
    virtual function void write(ram_seq_item t);
        ...
        `uvm_info(get_type_name(), $sformatf(
                  "ram_cg sampled: %s", item.conver2string()), UVM_MEDIUM)
    endfunction
    ...
    virtual function void report_phase(uvm_phase phase);
        ...
        `uvm_info(get_type_name(), $sformatf(
                  "     reset_n:  %.1f%%", ram_cg.cp_reset_n.get_coverage()),
                  UVM_LOW)
        `uvm_info(get_type_name(), $sformatf(
        ...
        `uvm_info(get_type_name(), $sformatf(
                  "     cross(rst,write):   %.1f%%",
                  ram_cg.cx_rst_write.get_coverage()
                  ), UVM_LOW)
        `uvm_info(get_type_name(), "\n\n===== Coverage Summary =====\n\n",
                  UVM_LOW)
    endfunction
...
class ram_scoreboard extends uvm_scoreboard;
    ...
    virtual function void write(ram_seq_item item);
        ...
        if (!item.reset_n) begin
            ...
            `uvm_info(get_type_name(), $sformatf("WRITE: addr=%0h wdata=%0h",
    ...
    virtual function void report_phase(uvm_phase phase);
        ...
        `uvm_info(get_type_name(), $sformatf(
                  "Total: %0d", match_count + error_count), UVM_LOW)
        ...
        if (error_count > 0)
            `uvm_error(get_type_name(), $sformatf(
                       "Test Failed: %0d mismatches!", error_count))
        else
            `uvm_info(get_type_name(), $sformatf(
                      "Test Passed: %0d all match!", match_count), UVM_LOW)
    endfunction
endclass
