...
class apb_scoreboard extends uvm_scoreboard;
    ...
    function void write(apb_seq_item tx);
        if (tx.p_write) begin
            ref_mem[tx.p_addr>>2] = tx.p_wdata;
            num_writes++;
        end else begin
            num_reads++;
            expected = ref_mem[tx.p_addr>>2];
            if (expected !== tx.p_rdata) begin
                num_errors++;
                `uvm_error(
                    get_type_name(),
                    $sformatf(
                        "FAIL! p_addr=0x%02h, expected=0x%08h, p_rdata=0x%08h",
                        tx.p_addr, expected, tx.p_rdata))
            end else begin
                `uvm_info(get_type_name(), $sformatf(
                          "PASS! p_addr=0x%02h, expected=0x%08h, p_rdata=0x%08h",
                          tx.p_addr,
                          expected,
                          tx.p_rdata
                          ), UVM_MEDIUM)
    ...
    virtual function void report_phase(uvm_phase phase);
        ...
        `uvm_info(get_type_name(), "*********** summary report ***********",
                  UVM_MEDIUM)
